...
export function initializeMainProcessAccountServices(): void {
  ...
  state.rateLimits.setIdealabUsageEnabledResolver(
    () => store.getSettings().idealabUsageEnabled === true
  )
  state.rateLimits.setZaiApiKeyResolver(() => store.getSettings().zaiApiKey)
