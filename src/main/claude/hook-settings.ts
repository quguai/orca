...
export function applyManagedHooks(
  ...
  const baseEvents: readonly ClaudeCompatibleHookEvent[] = options.events ?? CLAUDE_EVENTS
  const events = sessionEndCapable ? [...baseEvents, CLAUDE_SESSION_END_EVENT] : baseEvents
