...
export const detectHostedReviewProvider = async (context: ForgeProviderRepositoryContext) =>
  ((await getForgeProviderForRepository(context))?.id ?? 'unsupported') as ReviewProvider
