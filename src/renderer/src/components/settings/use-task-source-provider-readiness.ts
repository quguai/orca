...
import { useAoneStatus } from './aone-integration-cards'
...
export function useTaskSourceProviderReadiness(
  ...
  const { status: aoneStatus } = useAoneStatus()
  ...
  return useMemo(() => {
    ...
    return {
      ...
      aone: {
        connected: aoneStatus.kind === 'connected',
        checking: aoneStatus.kind === 'checking',
        unavailable: aoneStatus.kind === 'error',
        visible: visible.has('aone')
      },
      local: {
        connected: true,
        checking: false,
        visible: visible.has('local')
      }
    }
  }, [
    githubConnected,
