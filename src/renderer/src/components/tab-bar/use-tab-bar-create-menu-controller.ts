...
export type TabBarCreateMenuController = {
  ...
  launchAgentInline: (agent: TuiAgent) => void
...
export function useTabBarCreateMenuController({
  ...
  // Why: the inline strip launches outside the menu, so focus the new tab directly.
  const launchAgentInline = (agent: TuiAgent): void =>
    launchAgentTabFromEntry(agent, focusTerminalTabSurface, () => undefined)
