...
describe('automation host client', () => {
  ...
  it('encodes projectless global automations without repo or workspace selectors', () => {
    const automation = makeAutomation({
      kind: 'global_task',
      projectId: '',
      runContext: null
    ...
    // Why: a global automation has no project; an empty id must not become an
    // `id:` selector that a remote host would try to resolve.
    expect(toRuntimeAutomationCreateInput(input)).toMatchObject({
      kind: 'global_task',
      repo: undefined,
      workspace: undefined,
