...
export function getAutomationTargetAvailability({
  ...
  if (isGlobalScopedAutomation(automation)) {
    if (automationHostTarget?.kind === 'environment') {
      const runtimeAvailability = getRuntimeAutomationAvailability(
        automationHostTarget.environmentId,
        runtimeStatusByEnvironmentId
      )
    ...
    return (
      getAutomationSourceAvailability(automation.sourceContext, sourceHostAvailability) ?? {
        canRunNow: true,
        reason: 'available',
        message: null
      }
