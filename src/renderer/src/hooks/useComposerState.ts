...
export function useComposerState(options: UseComposerStateOptions): UseComposerStateResult {
  ...
  const handleRepoChange = useCallback(
    (
      ...
      // Why: Linear sources carry a canonical branch that must survive the repo switch.
  ...
  const handleProjectChange = useCallback(
    (projectId: string): void => {
      ...
      if (projectGroupId) {
        ...
        // Why: workspace-scoped sources (Linear/Jira/local tasks) survive a
        // project switch; repo-scoped links do not.
        const repoScopedLinkedItem =
          linkedProvider !== 'linear' && linkedProvider !== 'jira' && linkedProvider !== 'local'
        if (linkedWorkItem && repoScopedLinkedItem) {
          setLinkedWorkItem(null)
        }
        setSparseEnabled(false)
        setSparseDirectories('')
  ...
  const submit = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const submitShouldRunIssueAutomation =
        ...
        submitLinkedWorkItemProvider !== 'local' &&
