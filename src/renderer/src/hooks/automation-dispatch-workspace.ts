...
// Global-scoped runs (weekly_report/global_task) dispatch into the floating
// workspace, which has no real Worktree row — only id/displayName are read.
export type AutomationDispatchWorktree = Pick<Worktree, 'id' | 'displayName'>
...
export async function prepareAutomationDispatchWorkspace(args: {
...
}): Promise<AutomationDispatchWorktree | null> {
  ...
  const createResult =
    !isGlobalScoped && automation.workspaceMode === 'new_per_run'
      ...
      : null
  const worktree: AutomationDispatchWorktree | null = createResult
    ? (createResult.worktree ?? null)
    : isGlobalScoped
      ? {
