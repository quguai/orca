...
function isNewTurnEvent(source: AgentHookSource, eventName: unknown): boolean {
  ...
  switch (source) {
    // Why: Qoder and Kimi emit Claude-compatible hooks with the same new-turn boundary.
    case 'claude':
    case 'qoder':
...
function normalizeCodexEvent(
  ...
  // Why: Codex auto-allows request_user_input, so PreToolUse is the only signal that it is blocked on a human answer.
  ...
  if (eventName === 'Stop' && !hasCodexTranscriptSubagents(state, paneKey)) {
    // Why: Codex can omit child Stop hooks; later child activity safely recreates any agent still running.
    state.codexSubagentRosterByPaneKey.delete(paneKey)
  }
  const previousLead = state.codexLeadStateByPaneKey.get(paneKey)
  state.codexLeadStateByPaneKey.set(paneKey, {
