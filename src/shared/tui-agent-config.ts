...
export const TUI_AGENT_CONFIG: Record<TuiAgent, TuiAgentConfig> = {
  ...
  trae: {
    // Why: bytedance/trae-agent also installs `trae-cli`; `traecli` uniquely identifies TRAE CN's CLI.
    detectCmd: 'traecli',
    launchCmd: 'traecli',
    expectedProcess: 'traecli',
    promptInjectionMode: 'argv',
    // Why: prompts that resemble subcommands or flags must remain task text.
    argvPromptSeparator: '--'
  },
  opencode: {
    detectCmd: 'opencode',
