...
export function areWorkspaceLinkedItemsEqual(
  ...
  return (
    ...
    (a.localIdentifier ?? null) === (b.localIdentifier ?? null) &&
...
export function normalizeWorkspaceLinkedItem(value: unknown): WorkspaceLinkedItem | null {
  ...
  if (
    ...
    (raw.provider !== 'local' && raw.url.trim().length === 0) ||
    (raw.provider === 'local' &&
      (typeof raw.localIdentifier !== 'string' || raw.localIdentifier.trim().length === 0))
  ) {
    return null
  }
  return {
    ...
    ...(typeof raw.jiraIdentifier === 'string' && raw.jiraIdentifier.trim().length > 0
      ...
      : {}),
    ...(typeof raw.localIdentifier === 'string' && raw.localIdentifier.trim().length > 0
      ? { localIdentifier: raw.localIdentifier.trim() }
      : {}),
